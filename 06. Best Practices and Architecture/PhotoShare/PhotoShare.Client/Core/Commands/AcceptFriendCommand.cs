    ...
    public class AcceptFriendCommand
    {
        ...
        public static string Execute(string[] data)
        {
            ...
            using (var context = new PhotoShareContext())
            {
                ...
                if (alreadyAdded && accepted)
                {
                    throw new InvalidOperationException($"{addedFriendUsername} " +
                        $"is already a friend to {requesterUsername}");
                }

                if (!alreadyAdded && accepted)
                {
                    throw new InvalidOperationException($"{requesterUsername} has " +
                        $"already received a friend request from {addedFriendUsername}");
                }
